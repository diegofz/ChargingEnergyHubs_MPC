...
class EHv1(gym.Env):
    ...
    def __init__(self, battery_config, agent_config, data_dir='../Forecasting/EnergyHubEval/', env_mode='eval', do_save_env_vars=False):
        """
        ...
        :param data_dir: folder to read forecast, actual data
        :param env_mode: 'train' or 'eval'
        ...
        if self.forecast_mode == 'pred':
            ...
            self.ev_model = pickle.load(open(f'../Forecasting/Models/Pev_MapieGBR{self.forecast_model_version}_yearTest2021.pkl', 'rb'))
            self.pv_model = pickle.load(open(f'../Forecasting/Models/Ppv_MapieGBR{self.forecast_model_version}_yearTest2021.pkl', 'rb'))
            self.price_model = pickle.load(open(f'../Forecasting/Models/da_MapieGBR{self.forecast_model_version}_yearTest{self.priceYearTest}.pkl', 'rb'))
        ...
        self.grid_actual_save = self.load_actual_save = self.pv_actual_save = self.price_actual_save = self.reward_actual_save = None

        # initialize datasets
        self.all_data_ev = self.all_data_pv = self.all_data_price = None
    ...
    def step(self, action):
        ...
        :return reward: actual cost calculated as the actual power grid cost at time k (cost_g[k])
        ...
        load_actual, pv_actual, price_buy_now, price_sell_now = self._filter_actual_values(t_now=self.time_k)
    ...
    def _read_price_data(self):
        ...
        price = pd.read_csv(self.data_dir + f'AllTest_da_{self.priceYearTest}_v0.csv')
    ...
    def _init_episode_power_data(self):
        """
    ...
    def _filter_actual_values(self, t_now):
            ...
            return load_actual, pv_actual,  price_buy_actual, price_sell_actual
    ...
    def _filter_features(self, t_now):
        ...
        :return load_features_horizon: iltered load features
